...
import { VectorStoreBase } from './VectorStoreBase.mjs';
import { ProcessLock } from '../process-lock.mjs';

export class LanceDBManager extends VectorStoreBase {
  constructor(config, logger) {
    super(config, logger);
    this.db = null;
    this.tables = {};
    this._writeQueue = Promise.resolve(); // serializes all table mutations
    this._lock = new ProcessLock(path.join(this.configRoot, '.vmap', 'db'));
    process.on('exit', () => this._lock.release());
  ...
  _serialWrite(fn) {
    this._writeQueue = this._writeQueue.then(async () => {
      ...
      try { return await fn(); } finally { this._lock.release(); }
    }, async () => {
      await this._lock.acquire();
      try { return await fn(); } finally { this._lock.release(); }
    });
  ...
  sanitizePayload(payload) {
    ...
    for (const [key, value] of Object.entries(payload)) {
      ...
      if (typeof value === 'string') {
        ...
        sanitized[key] = JSON.stringify(value); // LanceDB: flatten nested objects
      }
    }
    return sanitized;
  }
  ...
  async getCollectionInfo(name) {
    ...
    try {
      ...
      return {
        points_count: count,
        config: { params: { vectors: { size: null } } }
      };
    } catch {
      return null;
    }
